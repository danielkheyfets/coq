...
Proof.
  ...
  induction contra; try discriminate.
    ...
    - inversion Heqloopdef. rewrite H1 in IHcontra2. rewrite H2 in IHcontra2. simpl in *. apply IHcontra2. simpl. reflexivity.
  Qed.
...
Inductive no_whilesR: com -> Prop :=
  | skip_no_whilesR : no_whilesR <{ skip }>
  | ass_no_whilesR : forall a b, no_whilesR<{ a:=b }>
  | seq_no_whilesR: forall (a b: com), no_whilesR a -> no_whilesR b -> no_whilesR <{a ; b}>
  | if_no_whilesR: forall (a: bexp) (b: com) (c:com), no_whilesR b -> no_whilesR c -> no_whilesR <{ if a then b else c end }>
  .

Theorem no_whiles_eqv:
   forall c, no_whiles c = true <-> no_whilesR c.
Proof.
  intro. split. induction c;
  try (simpl); try (intros); try (reflexivity);  try(apply skip_no_whilesR); try(apply ass_no_whilesR); try(apply seq_no_whilesR); try(apply if_no_whilesR); try (intros); try (apply andb_true_iff in H); try (discriminate H); try (destruct H);
  try (apply IHc1) ; try(apply IHc2); try( apply H ); try (apply H0).
  - induction c; intro H; inversion H; simpl; try(reflexivity).
...
Proof.
intros c . induction c; intros; inversion H; simpl in H.
- exists st. apply E_Skip.
- exists (x !-> aeval st b; st). apply E_Ass. rewrite H2. reflexivity. 
-  
  assert (exists st_new_1, st =[ c1 ]=> st_new_1) as [st_new_1 Hst'] by (apply IHc1; assumption);assert (exists st_new_2, st_new_1 =[ c2 ]=> st_new_2) as [st_new_2 Hst''] by (apply IHc2; assumption).
  exists st_new_2. apply E_Seq with st_new_1. apply Hst'. apply Hst''.
- simpl. inversion H. destruct (IHc1 st).
  -- apply H7.
  -- assert (exists st', st =[ c2 ]=> st') as [st2_new_2 Hst2'] by (apply IHc2; assumption);assert (exists st', st =[ c1 ]=> st') as [st1_new_1 Hst1'] by (apply IHc1; assumption).
  remember (beval st b) as bNew; simpl in *;destruct bNew.
  ---  exists st1_new_1;apply E_IfTrue.
    ** rewrite HeqbNew. reflexivity.
    ** apply Hst1'.
  --- exists st2_new_2. apply E_IfFalse.
    ** rewrite HeqbNew. reflexivity.
    ** apply Hst2'.
Qed.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Seq_break :
        ...
        st  =[ c1 ; c2 ]=> st' / SBreak
  | E_Seq_cont :
        forall c1 c2 st st' st'' s,
        st  =[ c1 ]=> st' / SContinue  ->
        st' =[ c2 ]=> st'' / s ->
        st  =[ c1 ; c2 ]=> st'' / s
  | E_IfTrue : 
        forall st st' b c1 c2 s,
        beval st b = true ->
        st =[ c1 ]=> st' / s->
  ...
  | E_While_True_break : forall st st'  b c,
        ...
        st  =[ c ]=> st' / SBreak ->
        st  =[ while b do c end ]=> st' / SContinue
  | E_While_True_cont : forall st st' st'' b c ,
        beval st b = true ->
        beval st'' b = false ->
        st  =[ c ]=> st' / SContinue ->
        st' =[ while b do c end ]=> st'' / SContinue ->
...
Proof.
intros. remember <{  while b do c end }> as t. induction H; inversion Heqt.
  - rewrite H2 in H. rewrite H0 in H. discriminate.
...
induction E1; intros st2 s2 E2; inversion E2; simpl; intros; try reflexivity; try (split; reflexivity);  try(discriminate);
try (rewrite H in H6; discriminate); try (apply (IHE1 st2 s2); apply H7); try (rewrite H in H2; discriminate). try (rewrite H in H5; discriminate).
- rewrite  H in H5. rewrite H5. split. reflexivity. reflexivity.
- apply (IHE1 SBreak s2). apply H4.
- apply IHE1 in H1. inversion H1. discriminate.
- apply IHE1_1 in H4. inversion H4. discriminate.
- apply IHE1_1 in H1. inversion H4. apply IHE1_2 in E1_2.  inversion H1. apply (IHE1_2 st2 s2).   rewrite  H7. apply H5.
- rewrite <- H4 in  H5. rewrite H in H5. discriminate.
- apply IHE1 in H6. inversion H6. rewrite H7. split. reflexivity. reflexivity.
- apply  IHE1 in H4. inversion H4. discriminate.
- rewrite <- H5 in H6. rewrite H6 in H. discriminate.
- apply IHE1_1 in H7. inversion H7. discriminate.
- apply IHE1_1 in H5. inversion H5. apply  (IHE1_2 SContinue s2). rewrite H10. apply H9.
Qed.
(** [] *)
End BreakImp.
...
(*i have two different aproaches 'Module ForImpByWhile' where i use while to implement the for and 'Module ForImp' where i imlement it from screach*)
(*in the 'Module ForImpByWhile' i prove again all the lemmas i proof in breakimp module. in this implementation it was important to have the break logic
st' =[ CWhile b (CSeq d c) ] because if we have break in the (d - body) we must stop the for and not proceead to c. and with the result logic while will do it for us for free.
*)
(*-------------------------in this module i use while and the break logic to implement for----------------------------------*)
Module ForImpByWhile.
        Inductive com : Type :=
        | CSkip
        | CBreak                      
        | CAss (x : string) (a : aexp)
      ...
      Notation "'while' x 'do' y 'end'" :=
              (CWhile x y)
                  (in custom com at level 89, x at level 99, y at level 99) : com_scope.
      Notation "'for' a ',' b ',' c ',' d 'end'" :=
        (CFor a b c d)
          (in custom com at level 89, a at level 99, b at level 99, c at level 99, d at level 99) : com_scope.
      ...
      (at level 40, c custom com at level 99, st' constr at next level).
            Inductive ceval : com -> state -> result -> state -> Prop :=
              ...
              | E_While_True_cont : forall st st' st'' b c ,
                    ...
                    st' =[ while b do c end ]=> st'' / SContinue ->
                    st  =[ while b do c end ]=> st'' / SContinue
              | E_For : forall st st' st'' a b c d,
                    st  =[ a ]=> st' / SContinue  ->
                    st' =[ CWhile b (CSeq d c) ]=> st'' / SContinue ->
                    st  =[ for a , b , c , d end]=> st'' / SContinue
          ...
          - apply IHE1_1 in H7. inversion H7. discriminate.
          - apply IHE1_1 in H5. inversion H5. apply  (IHE1_2 SContinue s2). rewrite H10. apply H9.
          -  apply IHE1_1 in H6. inversion H6. apply IHE1_2. rewrite H8. apply H7.  
          Qed.
End ForImpByWhile.
  

(*-------------------------in this module i implement it from screach----------------------------------*)
...
Module ForImp.
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ while b do c end ]=> st'' ->
      st  =[ while b do c end ]=> st''
  (* if the loop is condition is not satisfy in the begining *)
  | E_For_Start_Is_False_First_iteration : forall  st_a st_b a b c d,
  st_a =[ a ]=> st_b ->
      beval st_b b = false -> (*b is not satifyes so not go into the loop*)
      st_a =[ for a , b , c , d end ]=> st_b
  (* if the condition is ok! (a is initialized) *)
  | E_For_Start_Is_True_First_iteration : forall  st_a st_b st_c st_d  a b c d,
      st_a =[ a ]=> st_b -> (*a initialization i first time*)
      beval st_b b = true ->(*b is  satifyes so  go into the loop*)
      st_b =[ CSeq d c ]=> st_c ->  (*body*) (*last*)
      st_c =[ for skip , b , c , d end ]=> st_d -> (*in the next iteration no a !! so i use skip*)
      st_a =[ for a , b , c , d end ]=> st_d
      (*loop condition is false (not at the beginig)*)
  | E_For_Is_False_not_First_iteration : forall st_a b c d , 
      beval st_a b = false ->
      st_a =[ for skip , b , c , d end ]=> st_a
      (*cont with the loop*)
  | E_For_Is_True_not_First_iteration : forall  st_a st_b st_c  b c d, 
      (*b is  satifyes so  go into the loop again*)
      beval st_a b = true ->
      st_a =[ CSeq d c ]=> st_b  -> (*body*) (*last*)
      st_b =[ for skip ,  b , c , d end ]=> st_c ->
      st_a =[ for skip  , b , c , d end ]=> st_c
...
Definition loop : com :=
  ...
  Proof.
    ...
    - (* E_WhileTrue, b evaluates to true *)
      ...
      apply IHE1_2. assumption.
    - apply IHE1. assumption.
    -   apply IHE1 in H4.  rewrite <- H4 in H7. rewrite H7 in H. discriminate.
    - apply IHE1. apply  E_Skip.
    -  inversion E1. rewrite <- H2 in H. rewrite H in H6. discriminate.
    - apply IHE1_1 in H6. rewrite H6 in H. rewrite H in H7. discriminate.
    - auto. apply IHE1_3. apply IHE1_1 in H4. apply IHE1_2 in E1_2.
    apply IHE1_3 in E1_3. rewrite <- H4 in H8. apply IHE1_2 in H8. rewrite <- H8 in H9. apply  H9. 
    - inversion E1_1. rewrite  H2 in H6. rewrite H in H6. discriminate.
    - inversion E1_1. rewrite <- H2 in IHE1_2. apply IHE1_2 in H7. rewrite H7 in IHE1_3. apply IHE1_3 in H8. rewrite H8. reflexivity. 
    - inversion H6. reflexivity.
    - inversion H4. rewrite H2 in H. rewrite H in H7. discriminate.
    - reflexivity.
    - rewrite H3 in H. discriminate.
    - inversion H6. rewrite <- H2 in H7. rewrite H in H7. discriminate.
    -  inversion H4. rewrite <- H2 in H8. apply IHE1_1 in H8. rewrite <- H8 in H9. apply IHE1_2 in H9. rewrite H9 . reflexivity. 
    - rewrite H in H5. discriminate.
    - apply IHE1_1 in H6. rewrite <- H6 in H7. apply IHE1_2 in H7. apply H7.
    Qed.  
