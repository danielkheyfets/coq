...
Proof.
  ...
  -- simpl. apply MStarApp.
    ...
    -  apply IHss. intros. apply (H s). simpl. right. apply H0.
  Qed. 
(** [] *)
...
- generalize dependent s1.
  ...
  + intros. inversion H. rewrite H0. inversion H3. apply (IHs2 s3) in H4. rewrite <- H0. rewrite <- H5. simpl. rewrite H4. rewrite H7. reflexivity.  
- generalize dependent s1. induction s2 .
  + simpl. intros. rewrite H. apply MEmpty.
  + intros. rewrite H. unfold reg_exp_of_list. apply (MApp [x] _  s2 _ ) .
    -- apply MChar.
    -- simpl. apply  (IHs2 s2) . reflexivity.
  Qed.
(** [] *)
...
| App r1 r2  => (re_not_empty r1) && (re_not_empty r2) (*why not || *)
| Star r  => true
end.
...
Lemma re_not_empty_correct : forall T (re : reg_exp T),
  ...
  Proof.
  intros. split.
  - intros.  inversion H. induction H0 ;  try (simpl; reflexivity); simpl.
    + rewrite andb_true_iff. split; simpl;[apply IHexp_match1; exists s1;  apply H0_ | apply IHexp_match2; exists s2; apply H0_0 ].
    + simpl.  rewrite orb_true_iff. left. apply IHexp_match. exists s1. assumption.
    + simpl. rewrite orb_true_iff. right. apply IHexp_match. exists s2. assumption.
  - intros.  induction re ;simpl in H.
    + exfalso. discriminate.
    + exists []. apply MEmpty. 
    + exists [t].  apply MChar.
    + simpl. apply andb_true_iff in H. destruct H as [H1]. apply IHre2 in H. apply IHre1 in H1. inversion H1 as [l1 rol1]. inversion H as [l2 rol2]. simpl. exists (l1 ++ l2). apply MApp.  apply rol1. apply rol2.
    + simpl. apply orb_true_iff in H. destruct H as [H | H]; 
    [
      apply IHre1 in H ; destruct H as [s H]; exists s; apply MUnionL | 
      apply IHre2 in H ; destruct H as [s H]; exists s; apply MUnionR
    ]; assumption.
  + exists []. apply MStar0.
  Qed.
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp T),
  ...
  exists ss : list (list T),
    ...
    /\ forall s', In s' ss -> s' =~ re.
Proof.
intros. remember (Star re) as total. induction H; try (discriminate); simpl in *.
-  exists []. split.
  + simpl. reflexivity.
  + intros. simpl in H. exfalso. apply H.
- destruct (IHexp_match2 Heqtotal). exists (s1 :: x). split. destruct  H1.
  + simpl. rewrite H1. reflexivity.
  + intros s' HIn. destruct HIn as [HIn | HIn].
    * rewrite HIn in H. rewrite HIn in IHexp_match1.  injection Heqtotal. intros. rewrite <- H2.  assumption. 
    * destruct H1. apply H2. assumption. 
  Qed.
(** [] *)
...
(** The (weak) pumping lemma itself says that, if [s =~ re] and if the
    ...
    Lemma sum_pumping_constants_length_and_concat_help : forall (T : Type) (re1 re2: reg_exp T ) (s1 s2 : list T),
...
Lemma pumping_constants_res_length_t0_s1_help : forall T (re1 re2 : reg_exp T) (s1 : list T),
...
Lemma pumping_constants_res_length_to_s2_help : forall T (re1 re2 : reg_exp T) (s2 : list T),
pumping_constant re1 + pumping_constant re2 <= length s2 ->
...
Lemma not_empty_concat_help : forall T (s1 s2 : list T),
  1 <= length (s1 ++ s2) -> s1 ++ s2 <> [].
Proof.
  intros T s1 s2 Hlen contra.
  destruct s1 as [|x xs].
  - simpl in Hlen. inversion Hlen.
    + simpl in contra. rewrite contra in H0. simpl in H0. discriminate.
    + simpl in contra. rewrite contra in H. simpl in H. discriminate. 
...
Proof.
  ...
  - simpl. intros.  apply sum_pumping_constants_length_and_concat_help in H. destruct H.
    + destruct (IH1 H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3]. 
      exists x_1, x_2, (x_3 ++ s2). split.
        -- rewrite H1. repeat rewrite app_assoc. reflexivity.
        -- split; [ apply H2 |]. simpl. intros. repeat rewrite app_assoc. apply MApp.
          * rewrite <- app_assoc.  apply H3.
          * assumption.
    + destruct (IH2 H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3].
          exists (s1++x_1), x_2, x_3. split.
            -- rewrite H1. repeat rewrite app_assoc. reflexivity.
            -- split; [ apply H2 |]. simpl. intros. repeat rewrite <- app_assoc. apply MApp.
              * assumption.
              * apply H3.
  - intros. apply pumping_constants_res_length_t0_s1_help in H .   
    + destruct (IH H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3].
    exists x_1, x_2, x_3. split.
      --- rewrite H1. reflexivity.
      --- split.
        ++ apply H2.
        ++ intros. apply MUnionL. apply H3.
  - intros. apply pumping_constants_res_length_to_s2_help in H . 
    + destruct (IH H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3].
      exists x_1, x_2, x_3. split.
        --- rewrite H1. reflexivity.
        --- split.
          ...
          assert (Hp1 : pumping_constant re >= 1).
          { apply pumping_constant_ge_1. }
          assert (contra: 1<=0 -> False). {lia. }
          inversion Hp1 as [Hp1'| p Hp1' Hp1'']. 
          *** assert (HH : pumping_constant re >= 1). { apply pumping_constant_ge_1. } rewrite <- Hp1' in H. apply contra in H. exfalso. apply H.
          *** assert (HH: pumping_constant re <= 0 -> pumping_constant re >= 1 -> False). { lia. } apply  HH in H.
              **** lia.
              **** lia. 
  - intros Hlen. simpl in Hlen. simpl. exists [], ( s1 ++ s2), []. intros. split.
    -- simpl. rewrite app_nil_r. reflexivity.
    -- split.
      * assert (Hp1 : pumping_constant re >= 1). { apply pumping_constant_ge_1. } assert (pumping_constant re <=
      length (s1 ++ s2) -> pumping_constant re >= 1 -> 1 <= length (s1 ++ s2)). { lia. } apply H in Hlen. 
        ** apply not_empty_concat_help. apply Hlen.
        ** apply Hp1.
      * intros. simpl. induction m; simpl.
        ** apply MStar0.
        ** apply star_app; simpl.
          *** rewrite app_nil_r in IHm. apply star_app .
...
Lemma add_le_mono: forall a b c d : nat,
a <= b ->
c <= d ->
a + c <= b + d.
Proof.
...
Lemma nat_le_cases : forall n m : nat, n <= m \/ m <= n.
Proof.
  intros n m.
  lia.
Qed.
...
Proof.
  ...
  - simpl. intros contra. inversion contra.
  - simpl. intros contra. inversion contra. inversion H0.
  - simpl. intros. apply sum_pumping_constants_length_and_concat_help in H. assert(help : (pumping_constant re1 <= length s1 )  \/ ( length s1 <= pumping_constant re1) ) by apply nat_le_cases. destruct help as [help1 | help2].
    -- destruct H.
    + destruct (IH1 H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3]. destruct H3 as [H3 H4]. 
      exists x_1, x_2, (x_3 ++ s2). split.
        --- rewrite H1. repeat rewrite app_assoc. reflexivity.
        --- split; [ apply H2 |]. split.
          * assert (help: length x_1 + length x_2 <= pumping_constant re1 -> length x_1 + length x_2 <=
          pumping_constant re1 + pumping_constant re2 ) by lia. apply help. apply H3.
          * simpl. intros. repeat rewrite app_assoc. apply MApp.
            ** rewrite <- app_assoc.  apply H4.
            ** assumption.
    + destruct (IH1 help1) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3]. destruct H3 as [H3 H4]. 
            exists x_1, x_2, (x_3 ++ s2). split.
              --- rewrite H1. repeat rewrite app_assoc. reflexivity.
              --- split; [ apply H2 |]. split.
                * assert (help: length x_1 + length x_2 <= pumping_constant re1 -> length x_1 + length x_2 <=
                pumping_constant re1 + pumping_constant re2 ). {lia. } apply help. apply H3.
                * simpl. intros. repeat rewrite app_assoc. apply MApp.
                  ** rewrite <- app_assoc.  apply H4.
                  ** assumption.
    -- destruct H.
    ...
    exists x_1, x_2, (x_3 ++ s2). split.
      ...
      --- split; [ apply H2 |]. split.
        ...
        * simpl. intros. repeat rewrite app_assoc. apply MApp.
          ** rewrite <- app_assoc.  apply H4.
          ** assumption.
    + destruct (IH2 H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3]. destruct H3 as [H3 H4].
            exists (s1++x_1), x_2, x_3. split.
              --- rewrite H1. repeat rewrite app_assoc. reflexivity.
              --- split; [ apply H2 |]. split.
                ** simpl. rewrite -> app_length. rewrite <- plus_assoc. apply add_le_mono.
                ---- apply help2. 
                ---- apply H3 .
                ** simpl. intros. repeat rewrite <- app_assoc. apply MApp.
                  *** assumption.
                  *** apply H4.
  - intros. apply pumping_constants_res_length_t0_s1_help in H. 
    + destruct (IH H) as [x_1]. destruct H0 as [x_2]. destruct H0 as [x_3]. destruct H0 as [H1]. destruct H0 as [H2 H3]. destruct H3 as [H3 H4].
    exists x_1, x_2, x_3. split.
      --- rewrite H1. reflexivity.
      --- split.
        ++ apply H2.
        ++ split.
          +++ simpl. assert (ti : length x_1 + length x_2 <=
          pumping_constant re1 -> length x_1 + length x_2 <=
          pumping_constant re1 + pumping_constant re2). { lia. } apply (ti H3).
  ...
  - intros. apply pumping_constants_res_length_to_s2_help in H . 
    ...
    exists x_1, x_2, x_3. split.
      ...
      --- split.
        ...
        ++ split.
          +++ simpl.  assert (ti : length x_1 + length x_2 <=
          pumping_constant re2 -> length x_1 + length x_2 <=
          pumping_constant re1 + pumping_constant re2). { lia. } apply (ti H3).
  ...
  - simpl. destruct s1.
        + simpl; assumption.
        + remember (x :: s1) as l. assert (H_length_cases:(length l <= pumping_constant re) \/ (pumping_constant re <= length l) ) by apply nat_le_cases. simpl.
          destruct H_length_cases as [H_left | H_right].
          * simpl. intros.
          exists [], l, s2; simpl; split; [reflexivity |]; split; [unfold not; intros H1; rewrite Heql in H1; discriminate |]; split; [assumption |].
          intros. simpl. apply napp_star; assumption.
          *  simpl. apply IH1 in H_right. destruct H_right as [x_1]. destruct H as [x_2]. destruct H as [x_3]. destruct H as [H2 H3]. destruct H3 as [H4 H5]. destruct H5 as [H5 H6].
            exists x_1, x_2, (x_3 ++ s2).
            split.
              -- rewrite H2. repeat rewrite app_assoc. reflexivity.
              -- split; [assumption |]; split; [assumption |].
                  intros. repeat rewrite app_assoc.
                  rewrite <- (app_assoc _ x_1 (napp m x_2) x_3).
                  apply MStarApp; [apply H6 | assumption].
