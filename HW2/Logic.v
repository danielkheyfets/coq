...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H H0. unfold not. unfold not in H0. intros. apply H in H1. apply H0 in H1. destruct H1. Qed.
(** [] *)
...
Proof.
  intros X P H. unfold not. intros. destruct H0 . apply H0. apply H. Qed. 
(** [] *)
...
Proof.
  ...
  - induction l. intros H.
    ...
    + simpl. intros . destruct H .
      ...
      * apply IHl in H. destruct H as [xl [H1 H2]]. exists xl. split.
        -- apply H1.
        -- simpl. right. apply H2.
  - intros [x [H1 H2]]. induction l as [|a l' IHl'].
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros. induction l as [|a lb].
    - split.
      + intros H. simpl. reflexivity.
      + intros H x H0. simpl in H0. destruct H0.
    - split.
      + intros H. split.
        * apply H. left. reflexivity.
        * apply IHlb. simpl. intros. apply H. simpl. right. apply H0.
      + simpl. intros H x H0. simpl in H. destruct H. simpl in H0. destruct H0.
        * rewrite <- H0. apply H.
        * destruct  IHlb. apply H3.
          -- simpl. apply H1.
          -- simpl. apply H0.
Qed.
    (** [] *)
...
Theorem helper_tr_rev_correct: 
  forall X (l1 l2 : list X), @rev_append X l1 l2 = @rev_append X l1 [] ++ l2.
Proof.
  intros X. simpl. induction l1.
    - simpl. reflexivity.
    - simpl.  rewrite IHl1. simpl. intros. simpl. rewrite (IHl1 (x::l2)). rewrite <- app_assoc. simpl. reflexivity.
Qed.


Theorem tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof.
intros. apply functional_extensionality. intros.  unfold tr_rev. induction x as [| a b].
  - simpl. reflexivity.
  - simpl. rewrite <- IHb. simpl. apply helper_tr_rev_correct.
Qed.
(** [] *)
...
Proof.
  intros H X P H0 x. unfold not in H0. unfold excluded_middle in H. destruct (H (P x)).
    - apply H1.
    - simpl. unfold not in H1. exfalso. apply H0. exists  x. apply H1.
  Qed.
(** [] *)
...
(** **** Exercise: 5 stars, standard, optional (classical_axioms) 
    ...
    (*my solution*)
    (*peirce -> double_negation_elimination -> de_morgan_not_and_not -> implies_to_or -> peirce*)
    (*excluded_middle -> de_morgan_not_and_not -> excluded_middle*)
...
Theorem excluded_middle_to_de_morgan_not_and_not: excluded_middle -> de_morgan_not_and_not.
Proof.
intros. unfold excluded_middle in H. unfold de_morgan_not_and_not.
intros P Q H0. destruct (H P).
- left. apply H1.
- destruct (H Q). 
 * right. apply H2. 
 * unfold not in H0. unfold not in H1. unfold not in H2. destruct H0 . simpl. split.
 -- apply H1.
 -- apply H2.
Qed.
 Theorem a: de_morgan_not_and_not -> excluded_middle.
 Proof.
 intros H. unfold de_morgan_not_and_not in H. unfold excluded_middle. intros P. apply (H P (~P)). intros. unfold not. intros. destruct H0. apply H1 in H0. apply H0. 
Qed.
...
intros H P Q H0. destruct (H P P).
- intros H1. apply H1.
- unfold not in H1. apply H0. intros H2. apply H1 in H2. exfalso. apply H2.
- apply H1.
Qed.

Theorem peirce_to_double_negation_elimination: peirce -> double_negation_elimination.
Proof.
  intros H. unfold double_negation_elimination. unfold peirce in H. intros P H0. unfold not in H0. apply (H P False). intros H1. apply H0 in H1. exfalso. apply H1. 
Qed.


Theorem double_negation_elimination_to_de_morgan_not_and_not: double_negation_elimination -> de_morgan_not_and_not.
Proof.
  simpl. intros H. unfold double_negation_elimination in H. unfold de_morgan_not_and_not. intros.   apply (H (P \/Q)). simpl. unfold not. intros H1. apply H0. split. 
  +  unfold not. intros H2.  apply H1. left. apply H2.
  + unfold not. intros H2.  apply H1. right. apply H2.
Qed.

Theorem de_morgan_not_and_not_to_implies_to_or: de_morgan_not_and_not -> implies_to_or.
Proof.
simpl. intros H. unfold de_morgan_not_and_not in H. unfold implies_to_or. intros P Q H0. simpl in H. apply (H (not P) Q). unfold not. intros H1. destruct H1. apply H1. intros H3.  apply H2. apply H0. apply H3.
Qed.
