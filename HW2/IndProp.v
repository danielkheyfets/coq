...
Proof.
  ...
  - intros. simpl. inversion H. 
    + inversion H0.
      ...
      * simpl. apply ev_SS. rewrite  H3. apply H0.
    + rewrite H2. simpl. apply ev_SS. apply IHev.
  Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction H.
    ...
    -- unfold fR. simpl. rewrite <- plus_n_Sm. unfold fR in IHR. rewrite <- IHR. reflexivity.
    -- unfold fR in  IHR. simpl in IHR. rewrite  <- plus_n_Sm in IHR. unfold fR. injection IHR. intros. apply H0.
    -- unfold fR. unfold fR in IHR.   rewrite plus_comm. apply IHR.
  - intros.  generalize dependent n. generalize dependent o. induction m as [|v' d].
    + intros. simpl in H. rewrite H. generalize dependent n. induction o as [|g' d].
      -- intros. apply c1.
      -- intros. apply c3.  apply d with g' . reflexivity.
    + intros. rewrite <- H. apply c2. apply d. simpl. unfold fR. simpl. reflexivity.
  Qed.
...
(** The [ev] property provides a simple example for
    ...
    all, it is equivalent to the two non-in`ductive definitions of
...
| in_order_merge_nill : in_order_merge [] [] []

| in_order_merge_right: forall (n : X) (l1 l2 l : list X), in_order_merge l1 l2 l -> in_order_merge l1 (n :: l2) (n::l)

| in_order_merge_left: forall (n : X) (l1 l2 l : list X), in_order_merge l1 l2 l -> in_order_merge (n :: l1) l2 (n::l).

Lemma filter_challenge_helper : forall b : bool, negb b = true <-> b = false.
Proof.
  destruct b; split; intro H; try reflexivity; inversion H.
Qed.


Theorem filter_challenge: forall (X: Type) (l1 l2 l : list X) (test: X -> bool) ,
in_order_merge l1 l2 l ->((forallb test l1) = true) -> (forallb (fun n => negb (test n)) l2) = true ->  filter test l = l1.
Proof. 
intros X l1 l2 l test H H0 H1. induction H. simpl in H0. simpl in H1.
+ simpl. reflexivity.
+ simpl. unfold forallb in H1. apply andb_true_iff in H1. destruct H1 as [Hn Hl2].
  apply filter_challenge_helper in Hn. rewrite Hn.
  apply IHin_order_merge.
  - apply H0.
  - apply Hl2.
+ simpl. apply andb_true_iff in H0. destruct H0 as [Hn Hl1].
  rewrite Hn. f_equal.
  apply IHin_order_merge.
  - simpl.  apply Hl1.
  - apply H1.
...
| pal_many : forall x l, pal l -> pal ([ x ]  ++ l ++ [x ]).


Lemma pal_app_rev {X:Type} : forall l : list X, pal (l ++ rev l).
Proof.
  simpl. induction l.
  - simpl. apply pal_nil.
...
Lemma pal_rev {X:Type} : forall l : list X, pal l -> l = rev l.
Proof.
  simpl. intros l H.
  induction H.
  - reflexivity.
...
Inductive NoDup {X : Type} : list X -> Prop :=
  | NoDup_empty : NoDup []
  | NoDup_many : forall x l, ~ In x l -> NoDup l -> NoDup (x :: l).

  Lemma helper_NoDup : forall (X : Type) (x : X) (l1 l2 : list X),
...
Proof.
  ...
  - simpl. simpl in H. destruct H as [H | H].
    + left. left. apply H.
    + simpl. apply IHl1' in H. destruct H as [H | H].
      * left. right. apply H.
      * right. apply H.
Qed.
...
Theorem NoDup_interesting_theorems : forall (X : Type) (l1 l2 : list X),
  disjoint l1 l2 -> NoDup l1 -> NoDup l2 -> NoDup (l1 ++ l2).
Proof.
  intros .
  induction H0 as [| n l1' HH1 IHN1'].
  - simpl. apply H1.
  - simpl. apply NoDup_many.
    * simpl. unfold not in HH1. unfold not. intros h3. simpl. apply helper_NoDup in h3. destruct h3 as [H3 | H3].
      +  apply HH1. apply H3.
      + unfold disjoint in H. apply H in H3.
        -- apply H3.
        -- simpl. left. simpl. reflexivity.
    * simpl. apply IHIHN1'.
      -- unfold disjoint. intros. unfold not . intros. apply H in H2.
          --- apply H2.
...
- simpl in H. destruct H.
  + simpl. exists []. exists l'. simpl. rewrite H. reflexivity.
  + simpl. apply IHl' in H. destruct H as [l1 [l2 H']]. exists (y :: l1). exists l2. simpl. rewrite H'. reflexivity.
  Qed.
...
Inductive repeats  { X:Type } : list X  -> Prop  :=
    | repeats_x_in_l : forall (r:X) (l: list X), In r l  -> repeats (r :: l)

    | repeats_in_l : forall (r:X) (l: list X), repeats l -> repeats (r :: l).
...
(** Now, here's a way to formalize the pigeonhole principle.  Suppose
   ...
   - intros.
     ...
     apply repeats_in_l. apply (IHl1' l2).
     -- apply H.
     -- intros. apply (H0 x0). simpl. right. apply H3.
     -- simpl in H1. apply (IHl1' l2) in H.
     Admitted.
