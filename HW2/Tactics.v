...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n m o p. transitivity (minustwo o).
...
- intros m H. destruct m.
  + simpl. simpl in H. rewrite <- plus_n_Sm in H. discriminate H.
  + simpl. simpl in H. apply f_equal. apply h1'.
    rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H. injection  H as h1. apply h1. Qed.
  (** [] *) 
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  - intros. injection H. intros A B. rewrite <- B. rewrite <-A. simpl. reflexivity.
  - intros. generalize dependent l1. generalize dependent l2.   simpl. destruct (split t). simpl. destruct h. simpl. intros l0 l2  h.
   injection h. intros A B. rewrite <- A.  rewrite <- B. simpl.
  f_equal. simpl. apply hl. reflexivity. 
 Qed.
...
intros X test x l lf H. generalize dependent lf. 
  ...
  - intros lf H. simpl in H.  destruct (test a) eqn:k.
    + injection H. intros. rewrite <- H1. apply k. 
...
Proof. intros X test l. induction l as [| h l ih].
  + simpl. unfold existsb'. simpl. reflexivity.
  + simpl. rewrite ih. simpl. unfold existsb'. unfold forallb. destruct (test h).
    - simpl. reflexivity.
    - simpl. reflexivity.
    Qed. 
(** [] *)
