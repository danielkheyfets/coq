...
Definition ex_falso_quodlibet' : forall P, False -> P :=
  fun P (cont : False) => match cont with end.
(** [] *)
...
Proof.
intros. destruct H. apply H0. Show Proof. Qed. 
(** [] *)
...
Proof.
intros H P Q. unfold propositional_extensionality in H. apply (H (P\/Q) (Q\/P)). split.
...
Proof.
intros H P H0. unfold propositional_extensionality in H.
apply (H True P). split.
+ intros H1. apply H0.
...
Proof.
intros pe P pf1 pf2.
assert (True = P) as H. { apply (pe_implies_true_eq pe P pf1). } destruct H eqn:sav. destruct pf1. destruct pf2. reflexivity.
Qed.
(** [] *)
