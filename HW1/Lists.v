...
Definition member (v : nat) (s : bag) : bool := negb(eqb 0 (count v s)).
...
match s with
| [] => []
| t :: tx => match (eqb v t) with
            | true => tx
            | false => t :: (remove_one v tx)
            end
end.
...
| [] => []
| t :: tx => match (eqb v t) with
            | true => (remove_all v tx)
            | false => t :: (remove_all v tx)
            end
end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | [] => true
  | t :: tx => match (member t s2) with
              | true => (subset tx (remove_one t s2))
              | false => false
              end
  end.
...
Theorem helper : forall m , 
true = (leb m (S m) ) .
Proof.
 intros m. induction m as [|m' ih].
 - simpl. reflexivity.
 - simpl. rewrite ih. simpl. reflexivity. Qed.
...
(** How _many_ elements does the type [baz] have? (Explain in words,
    in a comment.)*)
(* Due to this recursive structure, it will have 0  elemnts. because baz will not have finet elemnts.
In the current definition of baz, there is no base case for the recursion, so we cannot construct a finite value of type baz.
 *)
