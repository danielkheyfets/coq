...
Proof.
  intros f x h1. destruct f eqn:Eb.
  - rewrite <- Eb. rewrite  x. rewrite x. reflexivity.
  - rewrite <- Eb. rewrite  x. rewrite x. reflexivity. Qed.
...
Theorem negation_fn_applied_twice :
  ...
  forall (b : bool ), f (f b) = b.
  intros f x h1. destruct f eqn:Eb.
  - rewrite <- Eb. rewrite  x. rewrite x. rewrite negb_involutive. reflexivity. 
  - rewrite <- Eb. rewrite  x. rewrite x. rewrite negb_involutive. reflexivity. Qed.
...
Fixpoint bin_to_nat (m:bin) : nat :=
   ...
   | Z => O
  | B1 Z =>  S O
  | B0 n => (bin_to_nat n) * S (S O) 
  | B1 n =>(bin_to_nat n) * S (S O)  + (S O) 
  end.
