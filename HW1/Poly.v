...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?  (Add YES or NO to each line.)
      - [d (b a 5)] no first val of d need be a type
      - [d mumble (b a 5)]  yes
      - [d bool (b a 5)] yes
      - [e bool true] yes
      - [e mumble (b c 0)] yes
      - [e bool (b c 0)] no
      - [c] no this is mumble not gumble *)
(* FILL IN HERE *)
End MumbleGrumble.
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
intros x l m n. induction l as [| n' ].
- reflexivity.
...
Proof.
  intros x l1 l2. induction l1 as [| n ].
...
Fixpoint split {X Y : Type} (l : list (X*Y))
  ...
  match l with
  | [] => ([] , [])
  | (x, y) :: ss => (x :: fst (split ss) , y:: snd (split ss))
  end.
...
Lemma helper : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl.   rewrite <- helper. rewrite ih. simpl. reflexivity. Qed.
(** [] *)
...
Proof.
  intros X Y Z f x y. simpl. reflexivity. Qed.

Theorem curry_uncurry : forall (X Y Z : Type)
                        (f : (X * Y) -> Z) (p : X * Y),
  prod_uncurry (prod_curry f) p = f p.
Proof.
intros X Y Z f p. simpl. destruct p as [x y]. reflexivity . Qed.
(** [] *)
